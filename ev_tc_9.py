...
def get_station_parameters(t, station_id):
    ...
    if station_id == 'S1':
        # Ph1: neutral | Ph2: aggressive cut to dominate O1 | Ph3: expensive | Ph4: moderate
        if   t < 125: return {'p_s': 0.55, 'mu_s': 1.2, 'c_s': 0.10, 'nu_s':  8.0}
        elif t < 250: return {'p_s': 0.15, 'mu_s': 1.5, 'c_s': 0.10, 'nu_s': 10.0}
        elif t < 375: return {'p_s': 0.65, 'mu_s': 2.0, 'c_s': 0.15, 'nu_s': 12.0}
        else:         return {'p_s': 0.45, 'mu_s': 2.5, 'c_s': 0.15, 'nu_s': 15.0}
    elif station_id == 'S2':
        # Ph1: neutral | Ph2: high price (S1 wins) | Ph3: aggressive cut to dominate O1 | Ph4: moderate
        if   t < 125: return {'p_s': 0.55, 'mu_s': 1.2, 'c_s': 0.10, 'nu_s':  8.0}
        elif t < 250: return {'p_s': 0.70, 'mu_s': 1.2, 'c_s': 0.10, 'nu_s':  8.0}
        elif t < 375: return {'p_s': 0.15, 'mu_s': 1.5, 'c_s': 0.12, 'nu_s': 10.0}
        else:         return {'p_s': 0.45, 'mu_s': 2.0, 'c_s': 0.15, 'nu_s': 13.0}
    elif station_id == 'S3':
        # Ph1: cheap (wins O2) | Ph2: moderate | Ph3: high | Ph4: moderate
        if   t < 125: return {'p_s': 0.25, 'mu_s': 1.8, 'c_s': 0.10, 'nu_s': 10.0}
        elif t < 250: return {'p_s': 0.35, 'mu_s': 1.8, 'c_s': 0.10, 'nu_s': 10.0}
        elif t < 375: return {'p_s': 0.60, 'mu_s': 2.2, 'c_s': 0.14, 'nu_s': 13.0}
        else:         return {'p_s': 0.40, 'mu_s': 2.5, 'c_s': 0.14, 'nu_s': 15.0}
    elif station_id == 'S4':
        # Ph1: Undercut S3 ($0.25) while maintaining profit margin
        if   t < 125: return {'p_s': 0.20, 'mu_s': 1.8, 'c_s': 0.10, 'nu_s': 10.0}
        # Ph2: Undercut S3 ($0.35) 
        elif t < 250: return {'p_s': 0.30, 'mu_s': 1.8, 'c_s': 0.10, 'nu_s': 10.0}
        # Ph3: S3 is expensive ($0.60). S4 undercuts profitably, NOT at a loss.
        elif t < 375: return {'p_s': 0.45, 'mu_s': 2.2, 'c_s': 0.12, 'nu_s': 13.0}
        # Ph4: Match/Slightly undercut S3 ($0.40)
...
#  KEY FIX: link_id is assigned AFTER the graph is built
#  by iterating G.edges() so link_id == enumerate-index
# ──────────────────────────────────────────────────────
def create_network():
    """
    Network topology (symmetric S1/S2 design):
    ─────────────────────────────────────────────────────────────────
    S1 sits at V2 (upper branch from O1):  O1──road──V2──[S1]──D1
    S2 sits at V1 (lower branch from O1):  O1──road──V1──[S2]──D1
    Both stations require exactly one road hop then one EV-only hop.
    This makes S1 and S2 structurally symmetric and competitive.

    S3/S4 on O2 side are already symmetric:
    S3: O2──[S3]──V3──road──D2 or D3
    S4: O2──road──V4──[S4]──D3
    ─────────────────────────────────────────────────────────────────
    """
    G = nx.MultiDiGraph()

    # Node insertion order determines G.edges() iteration order
    ...
    # ---- O1 roads: upper branch (via V2, S1) and lower branch (via V1, S2) ----
    G.add_edge('O1','V2', key=0, link_type='mixed')      # upper road O1->V2
    G.add_edge('O1','V1', key=0, link_type='mixed')      # lower road O1->V1

    # ---- V2 roads (S1 branch) ----
    G.add_edge('V2','D1', key=0, link_type='mixed')      # V2->D1 road (bypass S1)
    G.add_edge('V2','D1', key=1, link_type='EV-only',   station_id='S1')  # S1 at V2

    # ---- V1 roads (S2 branch) ----
    G.add_edge('V1','D1', key=0, link_type='mixed')      # V1->D1 road (bypass S2)
    G.add_edge('V1','D1', key=1, link_type='EV-only',   station_id='S2')  # S2 at V1
    ...
    # ---- Assign link_id = enumerate-index (CRITICAL: keeps link_id == array index) ----
    ...
    # Identify origin / destination / station link indices
    ...
    pos = {
        'O1': (0, 8),
        'V2': (5, 10),   # upper branch (S1 side)
        'V1': (5, 6),    # lower branch (S2 side)
        'D1': (10, 8),
        'O2': (0, 2),
        'V3': (7, 4),
        'V4': (3, 0),
...
def enumerate_paths(G, origins, destinations, charging_stations, idx_to_edge, edge_to_idx):
    ...
    for o_idx in origins:
        ...
        start = u_o   # origin node (O1 or O2, since it is a self-loop u==v)

        for d_idx in destinations:
            u_d, v_d, k_d = idx_to_edge[d_idx]
            end = u_d   # destination node
            ...
            if nx.has_path(G, start, end):
                for path_edges in nx.all_simple_edge_paths(G, start, end):
                    # skip paths that accidentally pass through self-loop edges
...
def compute_routing_matrix(y_EV, y_NEV,
                            ...
                            n_links):
    pair_demand = {}   # (j, i) -> total demanded flow on transition j->i
        ...
        for path in paths_dict[od]:
            ...
            for lid in path:
                ...
                if lt in ('origin','destination'):
                    ...
                    xi  = x[lid]
                    svc = outflow_fn(xi, mu=sp['mu_s'], nu=sp['nu_s'], is_charging=True)
                    wt  = xi / max(sp['mu_s'], 1e-9)  # paper eq(20): w = x_is/mu_s
...
def run_simulation():
    print("\n" + "="*70)
    print("EV CHARGING STATION COMPETITION SIMULATION  [FULLY FIXED]")
    print("="*70)

    (G, origins, destinations, charging_stations,
     idx_to_edge, edge_to_idx) = create_network()

    # Verify link_id == enum_idx
    for i, (u,v,k) in enumerate(idx_to_edge):
    ...
    # Helper: human-readable OD label
    def od_name(od):
    ...
    # Build active OD pair lists (only with demand AND paths)
    od_pairs_EV, y_EV_0   = [], []
    ...
    # At each phase boundary (pricing change), reinitialize path flows with an
    # exploration floor: each path gets at least MU_EXPLORE fraction of total demand.
    # This represents bounded rationality — drivers re-explore routes when prices change.
    MU_EXPLORE = 0.02   # 2% minimum exploration per path at each phase transition

    def apply_exploration_floor(state, od_pairs_EV, od_pairs_NEV,
                                paths_EV, paths_NEV, lambda_EV, lambda_NEV,
                                n_links, n_paths_EV):
        x_s  = np.maximum(state[:n_links], 0.0)   # clip negative densities
    ...
    phase_bounds  = [0, 125, 250, 375, T_FINAL]
    ...
    for ph in range(len(phase_bounds) - 1):
        ...
        # Reinitialize with exploration floor at each phase transition
        ...
        # Clip x >= 0 in output (physical constraint)
    ...
    # Print quick station summary
    print("\nStation queue means per phase:")
    for sid, lid in charging_stations.items():
        for _,ts,te in [('P1',0,125),('P2',125,250),('P3',250,375),('P4',375,500)]:
...
def create_network_animation(G, x_all, y_EV_all, y_NEV_all,
    ...
    # Parallel-edge map: (u,v) -> [key0, key1, ...]
    ...
    node_style = {
        'O1':('#2980b9','white'), 'O2':('#2980b9','white'),
        'D1':('#e74c3c','white'), 'D2':('#e74c3c','white'), 'D3':('#e74c3c','white'),
    }
    ...
    def draw_frame(frame):
        ax_net.clear()
        ax_net.set_xlim(-1, 13); ax_net.set_ylim(-1.5, 10)
    ...
    def render_info(frame):
        ...
        if ltype == 'EV-only':
            ...
            wt     = x_val / max(sp['mu_s'], 1e-9)  # paper eq(20): w = x_is/mu_s
    ...
    def find_nearest_link(mx,my,thresh=0.55):
        ...
        for u,v,k,edata in edges_list:
            ...
            x1,y1=pos[u]; x2,y2=pos[v]
            d = min(np.hypot(mx-px,my-py)
                    for px,py in arc_pts(x1,y1,x2,y2,rad))
            if d < best_d:
...
def plot_path_demands(t, y_EV_all, y_NEV_all,
    ...
    if not plots: print("  No path-demand data to plot."); return
                ...
                for path in paths:
                    ...
                    for lid in path:
                        ...
                        if lt in ('origin','destination'): continue
                            ...
                            xi  = x_t[lid]
                            svc = outflow_fn(xi,mu=sp['mu_s'],nu=sp['nu_s'],is_charging=True)
                            wt  = xi / max(sp['mu_s'], 1e-9)  # paper eq(20)
...
def plot_charging_station_metrics(q_s_traj, p_s_traj, t, charging_stations, x_traj):
    ...
    colors = {'S1':'#e74c3c','S2':'#3498db','S3':'#2ecc71','S4':'#f39c12'}
    ...
    x_pos = np.arange(len(phases)); width = 0.20
